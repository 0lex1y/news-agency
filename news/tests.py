...
class TopicsModelTest(TestCase):
    ...
    def test_topic_str(self):
        self.assertEqual(str(self.topic), 'War')  # перевірка чи правильно створюється назва теми
...
class RedactorModelTest(TestCase):
    def setUp(self):
        self.redactor = Redactor.objects.create_user(username='Tomkin',
                                                password='testpassword123',
                                                years_of_experience=5)
    ...
    def test_redactor_username_unique(self):
        with self.assertRaises(IntegrityError):
            Redactor.objects.create_user(username='Tomkin',
                                         password='testpassword123',
                                         years_of_experience=5)
...
class NewspaperListViewTest(TestCase):
    def setUp(self):
        ...
        self.newspaper = Newspaper.objects.create(title='War in Ukraine',
                                                  content='The War is in Ukraine start in 2022',
                                                  topic=self.topic)
        self.newspaper2 = Newspaper.objects.create(title='Peace in Ukraine',
...
class NewspaperDetailViewTest(TestCase):
    def setUp(self):
        ...
        self.newspaper = Newspaper.objects.create(title='War in Ukraine',
                                                  topic=self.topic,
                                                  content='The War is in Ukraine start in 2022',)
        self.publisher = Redactor.objects.create_user(username='Tomkin',
                                                      password='testpassword123',
                                                      years_of_experience=5)
        self.newspaper.publisher.add(self.publisher)

    def test_newspaper_status_code(self):
        response = self.client.get(reverse('news:news_detail', args=[self.newspaper.id]))
        self.assertEqual(response.status_code, 200)
    ...
    def test_newspaper_context(self):
        ...
        self.assertEqual(newspaper.content, 'The War is in Ukraine start in 2022')
